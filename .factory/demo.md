...
`mlr demo` starts a disposable Docker database on loopback. `mlr demo --dry-run` writes the same sample go/no-go card without Docker, for a quick documentation preview. `mlr demo --output ./mlr-demo --reset` removes only that explicit demo directory. Reset refuses the default report folder and working directories.
